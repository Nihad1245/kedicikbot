...
if (!client.commands.has(command)) {
    if (client.aliases.has(command)) {
      ...
      if(command == '') return;
    message.reply("Botda`" + command + '` Adında komut Tapılmadı')
  }
  }
   ...
   if (cmd.conf.enabled === false) {
      if (!ayarlar.sahip.includes(message.author.id) && !ayarlar.sahip.includes(message.author.id)) {
        const embed = new Discord.RichEmbed()
                    .setDescription(`:x: **${cmd.help.name}** Hal-hazırda o command işləmir!`)
                    .setColor("RED")
    ...
    if (cmd.conf.permLevel === 1) {
			if (!message.member.hasPermission("MANAGE_MESSAGES")) {
				const embed = new Discord.RichEmbed()
					.setDescription(`Bu commandı isdifadə etmək üçün **Mesajları Yönet** izninə sahib olmalısan!`)
          .setColor("RED")
				message.channel.send({embed})
				return
			}
