...
exports.run = async (client, msg, args) => {
  ...
  if (msg.channel.id !== '') return msg.channel.send(`${yasak} **Bu komutun İsdifadəsi Burda Əngəllənmişdir.**`).then(m => m.delete(5000));
  let u = msg.mentions.users.first() || msg.author;

        if(u.bot === true) {
                const embed = new Discord.RichEmbed()
                        .setDescription("Botların səvviyə olmur!")
                        .setColor("RANDOM")
 ...
 ctx.fill()
  ...
  var lvl = db.fetch(`seviye_${u.id + msg.guild.id}`);  
        ...
        ctx.fillText(`Səvviyə: ${lvl || 0}`, 160, 125);
        ctx.fillText(`Sıralama: ${sira}`, 160, 95);
        ctx.fillText(`Xal: ${xp || 0} / 150`, 195, 155);
        ctx.fillStyle = `#63fcf3`;
        ctx.font = '25px Impact';
        ctx.textAlign = "right";
        ctx.fillText(`BILGILƏR`, 170, 60);
        ctx.fillText(`RƏSIM`, 670, 50);
